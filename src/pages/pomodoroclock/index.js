...
const Pomodoroclock = () => {
  ...
  const SessionLength = 60 * 25;
  const [TimerLeft, setTimerLeft] = useState(SessionLength);
  const [Status, setStatus] = useState('New');
  const [Type, setType] = useState('Session');
  const [audio] = useState(
    new Audio('https://www.soundjay.com/clock/sounds/alarm-clock-01.mp3')
  );
  ...
  useEffect(() => {
    ...
    if (Status === 'Running') {
      timerID = setInterval(() => {
        ...
        if (newTimerLeft >= 0) {
          if (Type === 'Session' && newTimerLeft === 60 * 25 - 10)
            setPlaying(false);
          if (Type === 'Break' && newTimerLeft === 60 * 5 - 10)
            setPlaying(false);
    ...
    <div className={Styles.container}>
      <div className={Styles.main}>
        <h1 className={Styles.title}>
          {Status === 'New' && Type === 'Session'
            ? 'Can you focus in 25 minutes?'
            : Status === 'Running' && Type === 'Session'
            ? "You're doing great, keep up"
            : Status === 'Stop' && Type === 'Session'
            ? 'Just a little more to done, try hard'
            : "You are awesome, it's time for break"}
        </h1>
        <h1 className={Styles.time}>{Timedisplay(TimerLeft)}</h1>
      </div>
